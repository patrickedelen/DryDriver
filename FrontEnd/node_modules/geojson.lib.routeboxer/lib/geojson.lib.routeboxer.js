...
RouteBoxer.prototype.findIntersectingCells_ = function (vertices) {
  ...
  console.log('   -- hintXY[0]: ', vertices[0], hintXY);
  ...
  console.log('   -- gridXY['+i+']: ', vertices[i], vertices[i - 1], gridXY);
...
RouteBoxer.prototype.getCellCoords_ = function (Position) {
  for (var x = 0; this.lngGrid_[x] < Position[0]; x++) {}
  for (var y = 0; this.latGrid_[y] < Position[1]; y++) {}
  return ([x - 1, y - 1]);
};
...
RouteBoxer.prototype.getGridIntersects_ = function (start, end, startXY, endXY) {
  ...
  console.log('  -- bearingTo, rhumbBearingTo: ', geoLib.tools.bearingTo(start, end, 4), geoLib.tools.rhumbBearingTo(start, end, 4));
 ...
 *   line of latitude
 *
 * @param {GeoJSON.Position} start The vertex at the start of the path segment
 ...
 * @return {GeoJSON.Position} The latlng of the point where the path segment intersects
 *                    the grid line
 */
RouteBoxer.prototype.getGridIntersect_ = function (start, brng, gridLineLat) {
  var d = this.R * ((geoLib.tools.numberToRadius(gridLineLat) -
                     geoLib.tools.numberToRadius(start[1]) /
                     Math.cos(geoLib.tools.numberToRadius(brng))));

  return geoLib.tools.destinationPosition({type: 'Point', coordinates: start}, brng, d);
};
...
RouteBoxer.prototype.fillInGridSquares_ = function (startx, endx, y) {
  ...
  console.log('  => fillInGridSquares_: ', startx, endx, y);
